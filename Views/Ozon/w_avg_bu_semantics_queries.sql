...
WITH adapter AS (
    SELECT sd.project_id, sd.project_name, bu_id, bu_name, search_engine, 
      date, query_id, query_name,
      frequency2, avg_position, potential_traffic
...
, keys_with_projects AS (
    SELECT bu_id, search_engine, query_id, date, count(*) as projects
    FROM unique_keys
    GROUP BY bu_id, search_engine, query_id, date
    ORDER BY projects DESC, date DESC, bu_id, search_engine, query_id
)
...
, avg_position_prepared AS(
    SELECT bu_id, bu_name, date, search_engine, count(*) as query_count,
      sum(avg_position) as avg_position_sum,
      sum(frequency2) as sum_frequency2
    FROM prepared_data
    GROUP BY bu_id, date, search_engine, bu_name
    ORDER BY date DESC, bu_id, search_engine
)
SELECT bu_id, bu_name, date, search_engine, sum_frequency2,
    ROUND(avg_position_sum / query_count, 2) as avg_position_bu
FROM avg_position_prepared
GROUP BY bu_id, date, search_engine, bu_name, avg_position_sum, query_count, sum_frequency2
ORDER BY date DESC, bu_id, search_engine
;
