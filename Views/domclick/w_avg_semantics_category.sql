DROP VIEW IF EXISTS `seowork.domclick_views.w_avg_semantics_category`;

CREATE VIEW `seowork.domclick_views.w_avg_semantics_category` AS

WITH adapter AS (SELECT sd.project_id, project_name, project_description, 
      region_id, region_name, is_mobile, search_engine, date,
      queries_count, documents_count, category_id, category_name,
      frequency2, avg_position,
    ...
    FROM `seowork.domclick_views.v_semantics_category` as sd
)
   , unique_keys AS (
    SELECT project_id, search_engine, category_id, date, count(*) as `rows`
    FROM adapter
    GROUP BY project_id, search_engine, category_id, date
    HAVING `rows` = 1
...
ORDER BY project_id, category_name, search_engine, date
